...
def lambda_handler(event, context):
    ...
    rds_host = secretValues['host']
    name = secretValues['username']
    password = secretValues['password']
    db_name = secretValues['database']
    
    conn = pymysql.connect(rds_host, user=name, passwd=password, db=db_name, connect_timeout=5)
    with conn.cursor() as cur:
        cur.execute('INSERT INTO users(id,email,first_name,last_name,password) VALUES(%s,%s,%s,%s,%s)',(user[0],user[1],user[2],user[3],user[4]))
        conn.commit()
        cur.close()
